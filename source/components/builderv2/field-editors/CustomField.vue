...
<template>
  <div :key="name" class="field" :class="{required: field.required}">
    <label :for="`action_config_${name}`" class="action-config">
      {{humanize(name)}} <span v-if="field.secret">🔐</span>
    ...
    <div class="value-wrapper">
      <input :type="inputType" :id="`action_config_${name}`" class="code-entry" :class="{'ph-no-capture': field.secret}" autocomplete="off" :value="valueOf(config[name])" @input="input">
