        ...
        <div class="container">
            ...
            <div v-if="registrationEnabled">
                ...
                <form id='registerForm' @submit="handleSubmit">
                    <div>
                        <input type='text' class='text-field' placeholder ='Name' v-model='name' required/>
                    </div>
                    <div>
                        <input type='email' class='text-field' placeholder ='E-mail Address' v-model='email' required/>
                    </div>
                    <div>
                        <input type='password' class='text-field' placeholder='Password' v-model='password' required/>
                    </div>
                    <div>
                        <input type='checkbox' class='terms-checkbox' name='terms' v-model='termsAccepted' required>
                        I agree to the <a target="_blank" rel="noopener noreferrer" href="https://withtrivial.com/terms-of-service">Terms of Service</a> and <a target="_blank" rel="noopener noreferrer" href="https://withtrivial.com/privacy">Privacy Policy</a>
                    </div>  
                    <p v-if="errorMessage"><em>{{errorMessage}}</em></p>
                    <div class="submit">
                        <input v-if="!register_clicked" type='submit' class='button' value='Create Account' />
                        <input v-else type='submit' class='button clicked' value ='Creating...' />
                    </div>
                </form>
              </div>
              <div v-else>
                  ...
                  <form id="waitlistForm" @submit="handleJoinWaitlist">
                    ...
                    <div class="submit">
                        <input v-if="!join_waitlist_clicked" type='submit' class='button' value='Join Waitlist' />
                        <input v-else type='submit' class='button clicked' value ='Adding...' />
                    </div>    
                  </form>
              </div>
            <span class ="signIn">Already have an account? <a href="/signin">Sign In</a></span><br>
...
export default {
    ...
    methods: {
        ...
        handleJoinWaitlist(e) {
            ...
            TrackingService.track('User Joined Waitlist', {email: this.email, type: 'waitlist'})
